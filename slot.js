...
$(document).ready(function() {
	...
	*/
	...
	function printResult() {
		...
		for (i = 1; i < cnt; i++) 
		{ 
			result = result && (win[SlotArray[0].pos] === win[SlotArray[i].pos]);
		}
	...
	*/
	...
	$('#CreateBox').click(function() {
		...
		for (i = 0; i < cnt; i++) { 
			...
			SlotArray.push( new Slot('#'+id, 85, 6) );
		}
		
		$("<div/>", { "class": "clear"}).appendTo("#slot-container");
	});
	...
	$('#control').click(function() {
		...
		if(this.innerHTML == "Start") 
		{
			...
			x = window.setInterval(function() {		
				...
				for (i = 0; i < cnt; i++) 
				{ 
					if(SlotArray[i].speed >= SlotArray[i].maxSpeed) 
						count++;
				}
